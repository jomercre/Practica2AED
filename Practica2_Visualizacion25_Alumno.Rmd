...
grafico2<-ggplot(ds, aes(x = MinTemp, y = MaxTemp))+geom_point(aes(color=RainTomorrow))

print(grafico2)

grafico3<-ggplot(ds, aes(x = MinTemp, y = MaxTemp))+geom_point(aes(color=RainTomorrow)) + ggtitle('Relación entre temperaturas máxima y mínima y probabilidad de lluvia')
...
grafico2<-ggplot(ds, aes(x = Temp9am, y = Pressure9am)) + geom_point(aes(color=RainToday))

print(grafico2)

grafico3<-ggplot(ds, aes(x = Temp9am, y = Pressure9am)) + geom_point(aes(shape=RainToday))
...
print(grafico3)
```
Se observa mejor con los colores así que procedemos a modificar ese gráfico añadiendo título, subtítulo, leyenda y ejes adecuados.
```{r}
grafico4<-ggplot(ds, aes(x = Temp9am, y = Pressure9am)) + geom_point(aes(color=RainToday)) + labs(
    title = 'Presión frente a temperatura',
    subtitle = 'Análisis de la relación variables climáticas P y T para las 9am del dataset ds',
    x ='P (mbar)',
    y = 'T (ºC)')
...
iris.grafico2 <- ggplot(iris.tidy, aes(x = Species, y = Value)) + geom_point(aes(color=Part))

print(iris.grafico2)

iris.grafico3 <- ggplot(iris.tidy, aes(x = Species, y = Value)) + geom_point(aes(color=Part)) + facet_grid(. ~ Measure)

print(iris.grafico3)

iris.grafico4 <- ggplot(iris.tidy, aes(x = Species, y = Value)) + geom_point(aes(color=Part)) + facet_grid(Measure ~ .)

print(iris.grafico4)

ggsave('figs/Ejercicio3.pdf')
```
Los gráficos se comparan mejor el ponerlos en columnas distintas, es decir, al ponerlos en una misma fila, así que proseguimos con ese gráfico.
```{r}
iris.grafico5 <- ggplot(iris.tidy, aes(x = Species, y = Value)) + geom_jitter(aes(color=Part)) + facet_grid(Measure ~ .)
...
fig1 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas)) + geom_point(aes(color=Fabricante))

print(fig1)
```
Para observar ventas es mucho mejor utilizar un gráfico de rectas.
```{r}
fig2 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas)) + geom_point(aes(color=Fabricante)) + geom_line(aes(color=Fabricante))

print(fig2)

fig3 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas)) + geom_point(aes(color=Fabricante)) + geom_line(aes(linetype=Fabricante, color=Fabricante))

print(fig3)

fig4 <- ggplot(DatosEjer4, aes(x = Fecha, y = Ventas)) + geom_point(aes(color=Fabricante)) + geom_line(aes(linetype=Fabricante, color=Fabricante)) + labs(title='Ventas mensuales de diferentes compañías tecnológicas')
...
fig1 <- ggplot(Experimento, aes(x = Dosis, y = Altura)) + geom_point(color='Black',aes(shape=Complemento)) + geom_line(color='Black',aes(linetype=Complemento))

print(fig1)

fig2 <- ggplot(Experimento, aes(x = Dosis, y = Altura)) + geom_point(color='Black',aes(shape=Complemento)) + geom_line(color='Black',aes(linetype=Complemento)) + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura, linetype=Complemento), width=0.1,
    position=position_dodge(0.05))

print(fig2)

fig3 <- ggplot(Experimento, aes(x = Dosis, y = Altura)) + geom_line(aes(color = Complemento, linetype = Complemento)) + geom_point(aes(color = Complemento, shape = Complemento), size = 3) + geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura, color = Complemento), width=0.1,
    position=position_dodge(0.05)) +
...
fig1 <- ggplot(ice.river.df, aes(x=time)) + geom_line(color='Red', aes(y = flow.vat)) + geom_line(color='Blue',aes(y = flow.jok))

print(fig1)

fig2 <- ggplot(ice.river.df, aes(x=time)) + geom_line(color='Red', aes(y = flow.vat)) + geom_line(color='Blue',aes(y = flow.jok)) + labs(
    title = 'Caudales de los ríos islandeses Vatnsdalsa (azul) y Jokulsa Eystri (rojo)',
    x ='T (y)',
    y = 'Caudal (m^3/s)')
...
fig1 <- ggplot(ds, aes(x=Year,y=MinTemp)) + geom_boxplot()

print(fig1)

fig2 <- ggplot(ds, aes(x=Year,y=MaxTemp)) + geom_violin(aes(fill=Year)) + labs(
...
fig3 <- ggplot(ds, aes(x=Year,y=MaxTemp)) + geom_violin(aes(fill=Year)) + labs(
    title = 'Distribución de temperaturas máximas por año',
    x ='Año',
    y = 'Temperatura (ºC)')+theme(axis.text.x = element_text(
      angle = 45,hjust = 1,vjust = 1),legend.position = "none")
...
Cargamos readr para importar el txt.
```{r}
pacman::p_load(radr)
```

```{r Solucion ejercicio 8}
cholFile <- read_table("data/chol.txt")

fig1 <- ggplot(cholFile, aes(x=HEIGHT)) +
  geom_histogram()

print(fig1)

fig2 <- ggplot(cholFile, aes(x=HEIGHT)) +
  geom_histogram(aes(y = after_stat(density))) + labs(
...
fig3 <- ggplot(cholFile, aes(x=HEIGHT)) +
  geom_histogram(breaks=seq(155, 195, by=2), aes(y = after_stat(density))) + labs(
    title='Histograma de la variable Altura',
    x='Altura (cm)',
    y='Frecuencia'
  )
...
fig4 <- ggplot(cholFile, aes(x=HEIGHT)) +
  geom_histogram(breaks=seq(155, 195, by=2),
                 col="red",
                 fill="blue",
                 alpha = .5, aes(y = after_stat(density))) +
  labs(
    title='Histograma de la variable Altura',
    x='Altura (cm)',
    y='Frecuencia'
...
fig5 <- ggplot(cholFile, aes(x=HEIGHT)) +
  geom_histogram(breaks=seq(155, 195, by=2),
                 col="red",
                 fill="blue",
                 alpha = .5, aes(y = after_stat(density))) +
...
fig6 <- ggplot(cholFile, aes(x=HEIGHT)) +
  geom_histogram(breaks=seq(155, 195, by=2),
                 col="red",
                 fill="blue",
                 alpha = .5, aes(y = after_stat(density))) +
